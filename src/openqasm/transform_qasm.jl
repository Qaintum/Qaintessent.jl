    ...
    @match ctx_tokens begin
        ...
        Struct_gate(
            ...
            goplist = nothing && Do(goplist=[]) ||
                      goplist && Do(goplist = map(trans_gates, goplist))
         ) =>
        ...
        Struct_ifstmt(l=Token(str=l), r=r, gate_name=Token(str=gate_name), args=nothing, outs=outs) =>
            let l = Symbol(l),
                ...
                :($gate_name(nothing, reg_check($l,$r), $(refs...)))
            end
        ...
        Struct_ifstmt(l=Token(str=l), r=r, gate_name=Token(str=gate_name), args=exprlist, outs=outs) =>
            let l = Symbol(l),
                ...
                :($gate_name($exprs, reg_check($l,$r), $(refs...)))
            end
...
function transform_qasm(ctx_tokens)
    ...
    reg_declr = trans_reg(ctx_tokens)
    eval.(reg_declr)

    global qasm_cgc = CircuitGateChain(qregs, cregs)
    global N = size(qasm_cgc)

    gates_declr = trans_gates(ctx_tokens)
    eval.(gates_declr)
