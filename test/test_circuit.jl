...
@test_throws Exception CircuitGateChain{N}([
    ...
    single_qubit_circuit_gate(2, Y, N),
    controlled_circuit_gate((1, 3), 2, SGate(), N),
    controlled_circuit_gate((1, 3), 2, HadamardGate(), N; ccntrl=[2]),
    two_qubit_circuit_gate(2, 3, SwapGate(), N),
    single_qubit_circuit_gate(3, RxGate(1.5π), N),
    single_qubit_circuit_gate(3, RyGate(1.5π), N; ccntrl=[3]),
], creg=[0, 1])
...
@test_throws Exception CircuitGateChain{N}([
    ...
    single_qubit_circuit_gate(2, Y, N),
    controlled_circuit_gate((1, 3), 2, SGate(), N),
    controlled_circuit_gate((1, 3), 2, HadamardGate(), N; ccntrl=[0]),
    two_qubit_circuit_gate(2, 3, SwapGate(), N),
    single_qubit_circuit_gate(3, RxGate(1.5π), N),
    single_qubit_circuit_gate(3, RyGate(1.5π), N; ccntrl=[3]),
], creg=[0, 1, 0])
